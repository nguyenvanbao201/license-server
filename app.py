...
def _is_permanently_blocked(key, info=None, revoked=None):
    revoked = revoked if revoked is not None else load_revoked_keys()
    if key in revoked:
        return True
    if isinstance(info, dict):
        if info.get("revoked", False):
            return True
        if info.get("redeemed_once", False):
            return True
    return False
...
def _find_available_key(keys, requested_package=None, requested_duration_hours=None, requested_is_forever=False, revoked_keys=None):
    ...
    if requested_duration_hours is not None:
        for key, info in keys.items():
            ...
            if info.get("used", False) or info.get("issued", False) or _is_permanently_blocked(key, info, revoked_keys):
                continue
    ...
    for key, info in keys.items():
        ...
        if _is_permanently_blocked(key, info, revoked_keys):
            continue
...
def issue_key():
    ...
    key, info = _find_available_key(keys, requested_package, requested_duration_hours, requested_is_forever, revoked_keys)
...
def verify():
    ...
    if info.get("used", False) or info.get("revoked", False) or info.get("redeemed_once", False) or key in revoked_keys:
        revoked_keys[key] = {
            "revoked_at": info.get("used_at") or _now_iso(),
        ...
        return jsonify({
            "success": False,
            "message": "Key đã được sử dụng và bị khóa vĩnh viễn"
        })
...
def _count_available_keys_by_duration(keys):
    counts = {}
...
def key_counts():
    keys = load_keys()
    counts = _count_available_keys_by_duration(keys)
