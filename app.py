...
def _key_was_ever_activated(info):
    """Trả về True nếu key đã từng được kích hoạt/redeem ít nhất 1 lần."""
    if not isinstance(info, dict):
        return False

    return bool(
        info.get("used", False)
        or info.get("redeemed_once", False)
        or info.get("activated_at")
        or info.get("used_at")
        or info.get("telegram_activated_at")
    )
    ...
    if requested_duration_hours is not None:
        for key, info in keys.items():
            ...
            if (
                ...
                or _key_was_ever_activated(info)
...
def issue_key():
    ...
    return jsonify({
        ...
        "activated": _key_was_ever_activated(info),
        "expires_at": info.get("expires_at"),
    })
...
def telegram_verify_key():
    ...
    # MỖI KEY VIP CHỈ ĐƯỢC KÍCH HOẠT 1 LẦN DUY NHẤT.
    # Sau lần kích hoạt đầu tiên, mọi lần nhập lại đều bị từ chối,
    # kể cả chính Telegram ID đã kích hoạt trước đó.
    if _key_was_ever_activated(info):
        return jsonify({
            "success": False,
            "status": "TELEGRAM_VIP_ALREADY_ACTIVATED",
            "message": "Key VIP đã được kích hoạt trước đó và không thể kích hoạt lại",
            "key": key,
            "telegram_id": str(info.get("telegram_id") or ""),
            "activated_at": info.get("activated_at") or info.get("used_at") or info.get("telegram_activated_at"),
            "expires_at": info.get("expires_at"),
            "is_forever": _is_forever_key(info),
        })
