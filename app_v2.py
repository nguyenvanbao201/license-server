...
TELEGRAM_VIP_ACTIVATED_FILE = "telegram_vip_activated.json"
...
def telegram_verify_key():
    ...
    # Mỗi Telegram ID chỉ được kích hoạt VIP đúng 1 lần.
    if _telegram_vip_already_activated(telegram_id):
        return jsonify({
            "success": False,
            "status": "TELEGRAM_VIP_ALREADY_ACTIVATED",
            "message": "Telegram ID này đã từng kích hoạt Key VIP. Mỗi ID Telegram chỉ được kích hoạt VIP 1 lần.",
    ...
    _mark_telegram_vip_activated(telegram_id, key)
