...
def telegram_verify_key():
    ...
    # Khóa thêm theo Telegram ID nhưng KHÔNG đụng vào device hiện tại.
    ...
    if not bound_telegram_id:
        ...
        info["telegram_activated_at"] = _now_iso()
        changed = True

    # Không thay đổi used/device/activated_at của key nếu key đã kích hoạt
    # trên tool. Nếu key chưa dùng, vẫn kích hoạt được qua Telegram.
    if not info.get("used", False):
        now = _now()
        info["used"] = True
        info["redeemed_once"] = True
