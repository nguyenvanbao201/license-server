...
def _telegram_vip_already_activated(telegram_id):
    # Legacy helper: không dùng để chặn mua/kích hoạt thêm Key VIP.
...
def issue_key():
    ...
    # Chỉ Telegram Bot/nguồn quản trị được phép cấp key mua hàng.
    # Dùng cùng KEY_SERVER_ADMIN_SECRET với bot để tránh người ngoài gọi
    # /api/issue_key và tự ý lấy hết key trong kho.
    ...
    # 4 THÁNG đã bỏ khỏi bảng bán key, không cho phát hành qua API nữa.
    if (
        not requested_is_forever
        and (requested_duration_hours == 2880
             or "4 tháng" in _normalize(requested_package)
             or "4 thang" in _normalize(requested_package)
             or "4month" in _normalize(requested_package)
             or "4 months" in _normalize(requested_package))
    ):
        return jsonify({
            "success": False,
            "status": "PACKAGE_DISABLED",
            "message": "Gói 4 tháng đã được ngừng bán",
        }), 200
    ...
    if not key:
        package_label = str(requested_package or "Gói đã chọn").strip()
        return jsonify({
            "success": False,
            "status": "SOLD_OUT",
            "message": f"Gói {package_label} hiện đã được bán hết",
            "requested_package": requested_package,
            "requested_duration_hours": requested_duration_hours,
            "requested_is_forever": requested_is_forever,
        }), 200
...
def telegram_verify_key():
    ...
    # Cho phép một Telegram ID sở hữu nhiều Key VIP theo từng lần mua.
    # Quy tắc khóa mua nhiều lần được xử lý ở Telegram Bot: chỉ chặn khi
    # đang có đơn PENDING / WAITING_PAYMENT / WAITING_APPROVAL / PROCESSING.
    # Khi đơn trước đã COMPLETED hoặc REJECTED, user được tạo đơn mới dù
    # Key VIP cũ vẫn còn hạn. Vì vậy KHÔNG kiểm tra active_vip_key ở đây.

    # Khóa từng key theo Telegram ID. Một key đã gắn ID khác thì không thể dùng lại.
